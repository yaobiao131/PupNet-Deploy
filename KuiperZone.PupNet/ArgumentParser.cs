...
public partial class ArgumentParser
{
    ...
    private static readonly Regex StringSplitter = MyRegex();
    ...
    /// <summary>
    /// Gets leading floating values contained in the input. A floating value is one which lacks an
    /// argument key and, if present, must appear before options. Typically, it used to provide a
    /// command and/or path values. For example, given the input "command path --debug", the first
    /// index of <see cref="Values"/> will contain "command", while the second will contain "path".
    /// The "debug" option will be provided by <see cref="Keys"/> and <see cref="GetOrThrow"/> etc.
    /// The sequence is empty if the input lacks floating values.
    /// </summary>
    public IReadOnlyList<string> Values { get; }

    /// <summary>
    /// Gets the string at index 0 of <see cref="Values"/>, or returns null if <see cref="Values"/>
    /// is empty.
    /// </summary>
    public string? FirstValueOrNull
    {
        get { return Values.Count != 0 ? Values[0] : null; }
    }

    /// <summary>
    /// Gets the number of argument keys. This is synonymous with <see cref="Keys"/>.Count. For
    /// example, given the input "command --size=8 --debug", <see cref="Count"/> will be 2 rather
    /// than 3, as "command" is given in <see cref="Values"/>.
    /// </summary>
    public int Count
    {
