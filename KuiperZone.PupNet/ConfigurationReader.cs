...
public class ConfigurationReader
{
    ...
    public ConfigurationReader(PackageKind kind, bool examples, string? metabase = null)
    {
        ...
        if (examples)
        {
            ...
            PublisherCopyright = "Copyright (C) Acme Ltd 2025";
            ...
            IconFiles = ["app.ico", "app.svg", "app.16x16.png", "app.32x32.png", "app.64x64.png"];
            DotnetProjectPath = "Source/Acme.MyApp";
            DotnetPostPublish = "post-publish.sh";
            DotnetPostPublishOnWindows = "post-publish.bat";
            PackageName = "HelloWorld";
            AppImageArgs = "--sign --no-appstream";
    ...
    private ConfigurationReader(ArgumentReader args, IniReader reader, bool assertPaths)
    {
        ...
        PublisherId = GetOptional(nameof(PublisherId), ValueFlags.StrictSafe) ?? "";
        ...
        AppImageRuntimePath = GetOptional(nameof(AppImageRuntimePath), ValueFlags.AssertPath);
        ...
        // Added in Version 1.9 (ensure populated)
        if (PublisherId.Length == 0)
        {
            ...
            if (pos > 0)
            {
                PublisherId = AppId.Substring(0, pos);
            }
        }
    }
    ...
    public string AppId { get; } = "com.example.helloworld";
    ...
    public string PublisherId { get; } = "com.example";
    public string? PublisherCopyright { get; } = "Copyright (C) Hello World Team 2025";
    ...
    public string? AppImageRuntimePath { get; }
    ...
    public string ToString(DocStyles style)
    {
        ...
        sb.Append(CreateHelpField(nameof(PublisherId), PublisherId, style,
                $"Publisher ID in reverse DNS form. Invariably, this would be the same as {nameof(AppId)}, excluding the app leaf",
                $"name. The value populates the {MacroId.PublisherId.ToVar()} macro used AppStream metainfo. If omitted, defaults to",
                $"the leading parts of {nameof(AppId)}. It is highly recommended to specify the value explicitly."));
        ...
        sb.Append(CreateHelpField(nameof(AppImageRuntimePath), AppImageRuntimePath, style,
                ...
                $"the need for an internet connection during build, and fixes the runtime version. If {nameof(AppImageRuntimePath)}" ,
