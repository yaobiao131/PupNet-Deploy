...
public abstract class PackageBuilder
{
    ...
    /// <summary>
    /// Gets the EntryAssembly directory of this builder.
    /// </summary>
    public readonly static string SelfAssemblyDirectory = Path.GetDirectoryName(Assembly.GetEntryAssembly()?.Location) ??
    ...
    public static IReadOnlyCollection<int> StandardIconSizes { get; } =
        new List<int>([16, 24, 32, 48, 64, 96, 128, 256, 512, 1024]);
    ...
    /// <summary>
    /// Gets the architecture string tailed to suit the package builder. Subclass to override to set appropriate value
    /// based on <see cref="RuntimeConverter.BuildArch"/>. If user supplies <see cref="Arguments.Arch"/>, this value
    /// should be used verbatim.
    /// </summary>
    public abstract string PackageArch { get; }
    ...
    /// <summary>
    /// Gets output directory of assets associated with deployment package.
    /// </summary>
    public string OutputArtifactsDirectory
    {
        get { return Path.Combine(OutputDirectory, $"Artifacts.{Kind}.{PackageArch}"); }
    }
    ...
    /// <summary>
    /// Gets the source path of the "prime" icon, i.e. the single icon considered to be the most generally suitable. On
    /// Linux this is the first SVG file encountered, or the largest PNG otherwise. On Windows, it is the first ICO file
    /// encountered. It is a full path to the source icon on build.
    /// </summary>
    public string? PrimaryIcon { get; }

    /// <summary>
    /// A sequence of source icon paths (key) and their install destinations (value) under <see
    /// cref="BuildShareIcons"/>. Default generic icons are provided if the configuration supplies none. Always empty on
    /// Windows.
    /// </summary>
    ...
    private static List<string> GetDefaultIcons(bool terminal)
    {
        ...
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.16x16.png"));
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.24x24.png"));
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.32x32.png"));
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.48x48.png"));
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.64x64.png"));
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.96x96.png"));
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.128x128.png"));
        list.Add(Path.Combine(SelfAssemblyDirectory, $"{name}.256x256.png"));
