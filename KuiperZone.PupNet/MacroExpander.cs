...
public class MacroExpander
{
    ...
    public MacroExpander(PackageBuilder builder)
    {
        ...
        dict.Add(MacroId.PublisherId, conf.PublisherId);
        ...
        dict.Add(MacroId.PackageArch, builder.PackageArch);

        dict.Add(MacroId.BuildArch, builder.Runtime.BuildArch.ToString().ToLowerInvariant());
    ...
    /// <summary>
    /// Public for unit test only. Converts plain test with paragraphs separated by two or more empty lines to basic
    /// HTML. Also inserted &lt;ul&gt; items for lines beginning with "- " or "* ".
    /// </summary>
...
#if DEBUG
            throw new InvalidOperationException(varStr);
#endif
        }
    }
