...
public class BuildHost
{
    ...
    public BuildHost(ConfigurationReader conf)
    {
        ...
        {
            // AppId must have a '.'
            Builder.WarningSink.Add($"WARNING. Configuration item {nameof(Configuration.AppId)} should be in reverse DNS form, i.e. 'com.example.appname'");
        }
        ...
        if (Configuration.PublisherLinkUrl != null && !Configuration.PublisherLinkUrl.Contains("://") && !Configuration.PublisherLinkUrl.Contains('.'))
        {
            ...
            Builder.WarningSink.Add($"WARNING. Configuration item {nameof(Configuration.PublisherLinkUrl)} doesn't look like a valid URL (a valid example is: http://example.com)");
        }
