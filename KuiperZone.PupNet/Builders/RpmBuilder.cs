...
public sealed class RpmBuilder : PackageBuilder
{
    ...
    public override string OutputName
    {
        get
        {
            ...
            if (string.IsNullOrEmpty(output))
            {
                ...
                return $"{_rpmPackageName}_{AppVersion}-{PackageRelease}.{PackageArch}.rpm";
            }
    ...
    private string LocatePackageFilePath(string? dir = null)
    {
        ...
        dir ??= Path.Combine(_buildOutputDirectory, PackageArch);

        if (Directory.Exists(dir))
        {
            var expect = $"{_rpmPackageName}_{AppVersion}-{PackageRelease}.{PackageArch}.rpm";
    ...
    private string GetSpec()
    {
        ...
        sb.AppendLine($"BuildArch: {PackageArch}");
